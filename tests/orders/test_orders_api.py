...
from app.orders.models.order_product import (
    OrderProduct,
    OrderProductStatus,
...
class TestOrdersApi(BaseTestCase):
    def setUp(self):
        ...
        self.user = User(
            ...
            enabled=True,
            roles=[Role(name="allow_create_po")],
        )
        self.admin = User(
        ...
        self.try_add_entities(
            [
                ...
                ShippingRate(
                    shipping_method_id=1, destination="c1", weight=1000, rate=100
                ),
                ShippingRate(
                    shipping_method_id=1, destination="c1", weight=10000, rate=200
                ),
                NoShipping(id=999),
            ]
        )
    ...
    def test_create_order_wrong_input(self):
        ...
        self.assertEqual(res.status_code, 409)
        res = self.client.post("/api/v1/order", json={"customer_name": "User1"})
        self.assertEqual(res.status_code, 409)
        res = self.client.post(
            "/api/v1/order", json={"customer_name": "User1", "address": "Address1"}
        )
        self.assertEqual(res.status_code, 409)
        res = self.client.post(
            "/api/v1/order",
            json={"customer_name": "User1", "address": "Address1", "city_eng": "City1"},
        )
        self.assertEqual(res.status_code, 409)
        res = self.client.post(
            "/api/v1/order",
    ...
    def test_update_subcustomer_center_code(self):
        self.try_add_entities(
            [Product(id="0001", name="Product 1", price=10, weight=10)]
        )
        res = self.try_user_operation(
        ...
        assert Subcustomer.query.filter_by(username="A000").count() == 1
        assert Subcustomer.query.filter_by(username="A000").first().center_code == "111"
            ...
            operation=lambda: self.client.post(
                ...
                json={
                    ...
                    "suborders": [
                        {
                            "subcustomer": "A000, Subcustomer1, P@ssw0rd",
                            "items": [{"item_code": "0000", "quantity": "100"}],
                        }
                    ],
                },
            ),
    ...
    def test_create_order_from_draft(self):
        ...
        self.try_add_entities(
            [
                ...
                subcustomer,
                OrderProduct(
                    suborder=suborder, product_id="0000", price=10, quantity=10
                ),
            ]
        )
                    ...
                    "suborders": [
                        {
                            "subcustomer": "A000",
                            "seq_num": 1,
                            "items": [{"item_code": "0000", "quantity": 3000}],
                        }
                    ],
                    "params": {"p1": "P1"},
                },
    ...
    def test_get_orders(self):
        ...
        self.try_add_entities(
            [
                ...
                suborder,
                OrderProduct(
                    id=10, suborder=suborder, product_id="0000", price=10, quantity=10
                ),
            ]
        )
    ...
    def test_get_orders_by_age(self):
        ...
        self.try_add_entities(
            [
                ...
                so2,
                OrderProduct(
                    id=10, suborder=so1, product_id="0000", price=10, quantity=10
                ),
                OrderProduct(
                    id=20, suborder=so2, product_id="0000", price=10, quantity=20
                ),
            ]
        )
    ...
    def test_get_order(self):
        ...
        self.try_add_entities(
            [
                ...
                suborder,
                OrderProduct(
                    suborder=suborder, product_id=gen_id, price=10, quantity=10
                ),
    ...
    def test_get_order_products(self):
        ...
        suborder = Suborder(order=order, subcustomer=subcustomer)
        self.try_add_entities([order, subcustomer, suborder])
    ...
    def test_validate_subcustomer(self):
        ...
        self.try_add_entities([setting])
        res = self.try_user_operation(
            lambda: self.client.post(
                "/api/v1/order/subcustomer/validate",
                json={"subcustomer": "test, test, test"},
            )
        )
        self.assertEqual(res.status_code, 200)
        ...
        res = self.client.post(
            "/api/v1/order/subcustomer/validate",
            json={"subcustomer": "23426444, Mike, atomy#01"},
        )
        self.assertEqual(res.status_code, 200)
    ...
    def test_postponed_orders(self):
        ...
        self.try_add_entities(
            [
                ...
                suborder1,
                OrderProduct(
                    suborder=suborder1, product_id="0000", price=10, quantity=10
                ),
            ]
        )
    ...
    def test_set_status_shipped_twice(self):
        ...
        self.assertEqual(res.status_code, 200)
        self.assertEqual(self.user.balance, 0)
        self.client.post(f"/api/v1/admin/order/{gen_id}", json={"status": "shipped"})
        self.assertEqual(res.status_code, 200)
        self.assertEqual(self.user.balance, 0)
    ...
    def test_create_order_with_po(self):
        ...
        purchase_orders = PurchaseOrder.query.filter(
            PurchaseOrder.id.like(f"PO-{order_id[4:]}%")
        )
        self.assertEqual(purchase_orders.count(), 2)


class TestSubustomersAPI(BaseTestCase):
    def setUp(self):
        super().setUp()
        db.create_all()
        ...
        admin_role = Role(name="admin")
        self.user = User(
            username="user1_test_subcustomer_api",
            email="user1_test_subcustomer_api@name.com",
            password_hash="pbkdf2:sha256:150000$bwYY0rIO$320d11e791b3a0f1d0742038ceebf879b8182898cbefee7bf0e55b9c9e9e5576",
            enabled=True,
        )
        self.admin = User(
            username="root_test_subcustomer_api",
            email="root_test_subcustomer_api@name.com",
            password_hash="pbkdf2:sha256:150000$bwYY0rIO$320d11e791b3a0f1d0742038ceebf879b8182898cbefee7bf0e55b9c9e9e5576",
            enabled=True,
            roles=[admin_role],
        )
    ...
    @patch("app.orders.routes.subcustomer_api.get_bu_place_from_page")
    def test_get_center_code_from_page(self, mock_get_bu_place_from_page):
        mock_get_bu_place_from_page.return_value = "A000"
        res = self.try_user_operation(
            lambda: self.client.post(
                "/api/v1/order/subcustomer/center_code", json={"subcustomer": "A000, name, pass"}
            )
        )
        self.assertEqual(res.status_code, 200)
        self.assertEqual(res.get_json(), {"result": "success", "center_code": "A000"})

    @patch("app.orders.routes.subcustomer_api.get_bu_place_from_network")
    def test_get_center_code_from_network(self, mock_get_bu_place_from_network):
        mock_get_bu_place_from_network.return_value = "A000"
        res = self.try_user_operation(
            lambda: self.client.post(
                "/api/v1/order/subcustomer/center_code", json={"subcustomer": "A000, name, pass"}
            )
    ...
    def test_get_center_code_error(self):
        res = self.try_user_operation(
            lambda: self.client.post(
                "/api/v1/order/subcustomer/center_code", json={"subcustomer": "A000, name, pass"}
            )
        ...
        self.assertEqual(res.status_code, 404)
        self.assertEqual(res.get_json()['result'], "error") #type: ignore