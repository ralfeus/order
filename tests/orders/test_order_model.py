...
class TestOrderModelCurrency(BaseTestCase):
    ...
    def _make_order(self, user_currency_code='USD', total_base_currency=100000,
                    total_user_currency=100.0, shipping_base_currency=10000,
                    shipping_user_currency=10.0, subtotal_base_currency=90000,
        ...
        order.user_currency_code = user_currency_code
        order.total_base_currency = total_base_currency
        order.total_user_currency = total_user_currency
        order.shipping_base_currency = shipping_base_currency
        order.shipping_user_currency = shipping_user_currency
        order.subtotal_base_currency = subtotal_base_currency
    ...
    def test_get_total_no_currency_returns_krw(self):
        ...
        order = self._make_order(total_base_currency=100000, total_user_currency=100.0)
    ...
    def test_get_total_matching_user_currency_returns_precomputed(self):
        """get_total() with currency matching order.user_currency_code returns stored amount."""
    ...
    def test_get_total_arbitrary_currency_converts_from_krw(self):
        """get_total() with a currency different from order.user_currency_code converts on the fly."""
        order = self._make_order(user_currency_code='USD', total_base_currency=100000, total_user_currency=100.0)
    ...
    def test_get_total_none_user_currency_falls_back_to_conversion(self):
        ...
        order = self._make_order(user_currency_code='USD', total_base_currency=100000, total_user_currency=0)
        result = order.get_total(self.usd)
    ...
    def test_get_shipping_no_currency_returns_krw(self):
        order = self._make_order(shipping_base_currency=10000, shipping_user_currency=10.0)
    ...
    def test_get_shipping_user_currency_returns_precomputed(self):
        order = self._make_order(user_currency_code='USD', shipping_base_currency=10000, shipping_user_currency=10.0)
    ...
    def test_get_subtotal_no_currency_returns_krw(self):
        order = self._make_order(subtotal_base_currency=90000, subtotal_user_currency=90.0)
    ...
    def test_get_subtotal_user_currency_returns_precomputed(self):
        order = self._make_order(user_currency_code='USD', subtotal_base_currency=90000, subtotal_user_currency=90.0)
    ...
    def test_to_dict_includes_user_currency_code(self):
        """to_dict() must include user_currency_code field."""
        order = self._make_order(user_currency_code='USD', total_base_currency=100000, total_user_currency=100.0)
        result = order.to_dict()
        self.assertIn('user_currency_code', result)
        self.assertEqual(result['user_currency_code'], 'USD')
    ...
    def test_update_total_stores_user_currency_amounts(self):
        """update_total() computes user_currency amounts using order.user_currency_code rate."""
        product = Product(id='T001', name='Test', price=10000, weight=100)
        self.try_add_entity(product)
        order = Order(user=self.user, status=OrderStatus.pending)
        order.user_currency_code = 'USD'
        ...
        self.assertEqual(order.subtotal_base_currency, 22500)
        ...
        self.assertEqual(order.shipping_base_currency, 0)
        self.assertEqual(float(order.shipping_user_currency), 0.0)
        # total_base_currency = 22500
        self.assertEqual(order.total_base_currency, 22500)
    ...
    def test_update_total_base_currency_sets_zero_user_amounts(self):
        """update_total() when user_currency_code is base (KRW), user_currency amounts are 0."""
        product = Product(id='T002', name='Test2', price=10000, weight=100)
        self.try_add_entity(product)
        order = Order(user=self.user, status=OrderStatus.pending)
        order.user_currency_code = 'KRW'
        order.country = self.country
        order.shipping = self.no_shipping
        self.try_add_entity(order)
        suborder = Suborder(order=order)
        ...
        self.assertEqual(order.subtotal_base_currency, 12500)
