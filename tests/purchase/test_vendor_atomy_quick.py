...
class TestPurchaseOrdersVendorAtomyQuick(BaseTestCase):
    ...
    @patch(
        "app.purchase.models.vendors.atomy_quick.invoke_curl",
        MagicMock(side_effect=invoke_curl)
    )
    @patch(
        "app.purchase.models.vendors.atomy_quick:AtomyQuick._AtomyQuick__login",
        MagicMock(return_value=["JSESSIONID=token"]),
    )
    @patch(
        "app.purchase.models.vendors.atomy_quick.get_json",
