...
class AtomyQuick(PurchaseOrderVendorBase):
    ...
    def __set_purchase_date(self, page: Page, sale_date: date):
        def is_date_set():
            ...
            for _ in range(5):
                if page.locator('[order-role="sale-date"]').text_content() == sale_date_str:
                    ...
                    return
                self._logger.debug("Expected %s, got %s. Waiting...",
                                   sale_date_str, page.locator('[order-role="sale-date"]').text_content())
                sleep(1)
        ...
        if sale_date:
            ...
            try:
                self._logger.debug(sale_date_loc.inner_html(timeout=1000))
            except:
                pass
    ...
    def __set_receiver_mobile(self, page: Page, phone="     "):
        ...
        try:
            ...
            if phone_loc.input_value() == '':
                self._logger.warning("Couldn't set receiver phone number. Retrying")
                return self.__set_receiver_mobile(page, phone)
