...
function format ( row, data ) {
    var order_product_history = get_history(data.order_product_id, function(history_data) {
    ...
    return '<div class="container product-details">'+
        '<div class="row container">'+
            '<label class="col-3" for="private_comment">Private comment:</label>'+
            '<label class="col-3" for="public_comment">Public comment:</label>' +
            '<input type="button" class="button btn-primary btn-save col-1" value="Save" />' +
        '</div>' +
        '<div class="row container">' +
            '<textarea id="private_comment" class="form-control col-3">' + data.private_comment + '</textarea>' +
            '<textarea id="public_comment" class="form-control col-3">' + data.public_comment + '</textarea>' +
        '</div>'+
        '<div class="row container">' +
            '<select id="status_history" multiple></select>' +
        '</div>' +
    '</div>';
}
