...
async function get_dictionaries() {
    g_invoice_templates = await get_list('/api/v1/invoice/template');
}
...
function get_excel(rows) {
    ...
    modal("Choose the invoice template", `
        ...
        .then(form => {
            ...
            $('.wait').show();
            if (rows.count() == 1) {
                window.open(`/api/v1/invoice/${rows.data()[0].id}/excel?template=${form.template}`);
            } else {
                var invoices = '';
                for (var i = 0; i < rows.count(); i++) {
                    invoices += `invoices=${rows.data()[i].id}&`;
                }
                window.open(`/api/v1/invoice/excel?${invoices}&template=${form.template}`);
            }
            $('.wait').hide();
        });
}
