...
def admin_get_subcustomers():
    ...
    if request.values.get("q") is not None:
        subcustomers = Subcustomer.get_filter(
            subcustomers, filter_value=request.values["q"]
        )
        ...
        return jsonify(
            {
                ...
                "pagination": {"more": total_results > page * 100},
            }
        )
...
def admin_create_subcustomer():
    ...
    try:
        ...
        if node is None:
            _invoke_node_api(
                "/api/v1/node/" + payload["username"], method="put", data=payload
            )
        else:
            _invoke_node_api(
                "/api/v1/node/" + payload["username"], method="patch", data=payload
            )
...
def admin_save_subcustomer(subcustomer_id):
    ...
    if (
        ...
        and Subcustomer.query.filter_by(username=payload["username"]).count() > 0
    ):
        abort(
            Response(
...
def admin_delete_subcustomer(subcustomer_id):
    ...
    if owned_suborders.count() > 0:
        ...
        abort(
            Response(
                f"Can't delete subcustomer that has suborders: {suborder_ids}",
                status=409,
            )
        )
    db.session.delete(subcustomer) #type: ignore
...
def validate_subcustomer():
    if not Setting.get("order.new.check_subcustomers"):
        return {
            "result": "success",
            "message": "No subcustomer check is performed",
        }, 200
    payload = request.get_json()
...
@bp_api_user.route("/subcustomer/center_code", methods=["POST"])
@login_required
def get_center_code():
    """Get center code for the current user"""
    logger = current_app.logger.getChild("get_center_code")
    payload = request.get_json()
    if not payload or not payload.get("subcustomer"):
        abort(Response("No subcustomer data was provided", status=400))
    try:
        subcustomer, _is_new = parse_subcustomer(payload["subcustomer"])
        if subcustomer.center_code:
            return jsonify({"result": "success", "center_code": subcustomer.center_code})
        
        code = get_bu_place_from_page(subcustomer.username, subcustomer.password)
        return jsonify({"result": "success", "center_code": code})
    except SubcustomerParseError as ex:
        return jsonify({"result": "error", "message": str(ex)})
    except Exception as ex:
        logger.info(
            "Couldn't get center code for subcustomer %s from page. Trying to get from network",
            subcustomer.username,
        )
        try:
            code = get_bu_place_from_network(subcustomer.username)
            return jsonify({"result": "success", "center_code": code})
        except Exception as e:
            logger.error(
                "Couldn't get center code for subcustomer %s: %s",
                subcustomer.username,
                str(e),
            )
            return jsonify({"result": "error", "message": str(e)}), 404

    return jsonify({"center_code": subcustomer.center_code})
